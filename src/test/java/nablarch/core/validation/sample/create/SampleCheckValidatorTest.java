...
public class SampleCheckValidatorTest {
    ...
    public void testValidateFail() {
        ...
        assertEquals("PROP0001は \"0002\", \"0003\" のいずれかで入力してください。", context.getMessages().get(0).formatMessage());
    }
    ...
    public void testValidateAllow0001Fail() {
        ...
        ThreadContext.setLanguage(Locale.JAPANESE);
        assertEquals("PROP0001は \"0001\", \"0002\", \"0003\" のいずれかで入力してください。", context.getMessages().get(0).formatMessage());
    }

    @Test
    public void testWithManager() {
    	ValidationManager manager = (ValidationManager) SystemRepository.getObject("validationManager");
    	BasicStaticDataCache stringResourceCache = (BasicStaticDataCache) SystemRepository.getObject("stringResourceCache");
    	BasicStaticDataCache formDefinitionCache = (BasicStaticDataCache) SystemRepository.getObject("validationManager.formDefinitionCache");
    	
    	formDefinitionCache.initialize();
    	stringResourceCache.initialize();
    	manager.initialize();
    	
    	{
	    	...
	    	ValidationContext<TestEntity> result = ValidationUtil.validateAndConvertRequest("", TestEntity.class, params, "validateAll");
